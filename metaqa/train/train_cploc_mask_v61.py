    ...
    def __init__(self, is_training=True, clue_level=1):
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope("decoder"):
                ...
                ## Dropout
                self.dec_word = self.dec = tf.layers.dropout(self.dec, 
                                            rate=hp.dropout_rate, 
                                            training=tf.convert_to_tensor(is_training))
                ...
                for i in range(hp.num_blocks):
                    with tf.variable_scope("num_blocks_{}".format(i)):
                        ...
                        #                                dropout_rate=hp.dropout_rate,
                        #                                is_training=is_training,
                        #                                causality=False if i<1 else True, 
                        #                                cloze=True if i<1 else False, 
                        ...
                        self.dec = multihead_attention(queries=self.dec, 
                                                       ...
                                                       cloze=True, 
                                                       scope="self_attention")
                        tf.add_to_collection('save_op', io_ops._save(filename="tmp1/dec1_{}.ckpt".format(i),
                                 tensor_names=["dec"], tensors=[self.dec]))
                        ...
                        tf.add_to_collection('save_op', io_ops._save(filename="tmp1/dec2_{}.ckpt".format(i),
                                 tensor_names=["dec"], tensors=[self.dec]))
